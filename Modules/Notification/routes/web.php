...
use Modules\Notification\Http\Controllers\Admin\FirebaseTokenController;
use Modules\Notification\Http\Controllers\Admin\NotificationTemplateController;
...
Route::controller(FirebaseTokenController::class)->group(function () {
    Route::post('save-fcm-token', 'saveFcmToken')->middleware('active.admin')->name('saveFcmToken');
    Route::get('firebase-messaging-sw.js', 'serviceWorker')->name('firebaseServiceWorker');
});

// NotificationTemplate Crud Section
Route::prefix('notification-templates')->name('notification_templates.')->controller(NotificationTemplateController::class)->group(function () {
...
Route::prefix('notifications')->name('notifications.')->controller(NotificationController::class)->group(function () {
    Route::get('list'                                   , 'index')->name('index');
    Route::get('datatable'                              , 'datatable')->name('datatable');
    Route::get('admin-web-notifications'                , 'adminWebNotifications')->name('adminWebNotifications');
    Route::post('update-notification-status'            , 'updateNotificationChannelStatus')->name('updateNotificationChannelStatus');
    Route::get('create'                                 , 'create')->name('create');
    Route::post('create'                                , 'postCreate')->name('postCreate');
    Route::delete('soft-delete/{model}'                 , 'softDelete')->name('softDelete');
    Route::delete('hard-delete/{model}'                 , 'hardDelete')->name('hardDelete');
